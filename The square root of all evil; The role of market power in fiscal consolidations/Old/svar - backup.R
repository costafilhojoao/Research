...
PY  <- subset( base4,  na_item == "B1GQ" )        # Gross domestic product at market prices
PC  <- subset( base4,  na_item == "P31_S14_S15" ) # Household and NPISH final consumption expenditure
PI  <- subset( base4,  na_item == "P5G" )         # Gross capital formation
PG  <- subset( base4,  na_item == "P3_S13" )      # Final consumption expenditure of general government
PX  <- subset( base4,  na_item == "P6" )          # Exports of goods and services
PM  <- subset( base4,  na_item == "P7" )          # Imports of goods and services
...
pop   <- window( ts( pop$values, start=c(1998,1), frequency = 4) , start = c( 1999, 1), end = c( 2019, 4) ) * 1000

# Per capita figures

ypc = Y * 1000000000 / pop
cpc = C * 1000000000 / pop
gpc = G * 1000000000 / pop
ipc = I * 1000000000 / pop
xpc = X * 1000000000 / pop
mpc = M * 1000000000 / pop
...
gspc = GS * 1000000000 / pop
tpc  = T * 1000000000 / pop
...
var <- VAR( data, p = 1, type =  "const", season = NULL, exogen = NULL, lag.max = NULL, ic = HQ )
#summary(var)
...
Amat[1, 1] <- 1
Amat[1, 2] <- 0 # sem surpresas - o governo controla essas variÃ¡veis
Amat[1, 3] <- 0 #alpha g,y - elasticidade dos gastos ao pib - sem efeito contemporÃ¢neo (dados trimestrias)
Amat[2, 1] <- 0 # sem surpresas - o governo controla essas variÃ¡veis
Amat[2, 2] <- 1
Amat[2, 3] <- -1.2 #alpha y,t - elasticidade dos tributos ao PIB
#Amat[3, 1] <- #alpha t,y (elasticidade do pib aos tributos)
#Amat[3, 2] <-   #alpha g,y (elasticidade do pib aos gastos)
Amat[1, 4] <- 0
Amat[3, 3] <- 1 
Amat[4, 4] <- 1 
Amat[1, 5] <- 0
